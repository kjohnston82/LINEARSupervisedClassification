...
source("src/astro/utils/classification/GenerateRFAndROCCurvesOptimum.R")
...
for(i in 1:length(classType)){
	...
	performanceCurves = GenerateRFAndROCCurvesOptimum(
	...
	#	probTemp   = predict(tmpClassifier , newdata = sampleLinear , type='prob')
...
#save(predTestProb, file = "data/PredProb_RF.R")

save(tmpFalseRateArray, tmpHitRateArray, tmpPrecisionRateArray, 
	file = "data/PerformanceStatistcsRF.R")