GenerateRFAndROCCurves <- function(
	Training.data, Training.label, Crossval.data, Crossval.label,
	summaryType) {
  
mtryArray<- seq(from = 1, to = 21, by = 5);
nTreeArray<- seq(from = 100, to = 300, by = 100);
alphaCrit <- sort(seq(from = -0.01, to = 1.01, by = 0.01), decreasing = TRUE);

hitRate  		= array(dim=c(length(nTreeArray),length(alphaCrit)));
falseRate 		= array(dim=c(length(nTreeArray),length(alphaCrit)));
precisionRate 	= array(dim=c(length(nTreeArray),length(alphaCrit)));

for(k in 1:length(nTreeArray)){

	rf.tr = randomForest(
		x = as.matrix(Training.data),
		y = factor(Training.label),
		mtry = 8, ntree = nTreeArray[k])
...
legend("bottomright", legend = nTreeArray, col = c(2,3,4), lty = c(1,3,4), cex = 0.8, title = "gamma")
grid()
