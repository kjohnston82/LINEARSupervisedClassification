...
source("src/astro/util/utils_classify.R")
####################################################
####################################################
# load library
...
linear.bvr = color.transform(ra.dec.linear$uMod,ra.dec.linear$gMod,
ra.dec.linear$rMod, ra.dec.linear$iMod,ra.dec.linear$zMod)
...
## linear.eb = read.table("data/trainingsets/EBs_IDs.txt",header=FALSE)[,1]
## linear.rrc = read.table("data/trainingsets/data_list_RRc.txt",header=TRUE)[,1]
## linear.rrab = read.table("data/trainingsets/RRab.dat",header=FALSE)[,1]
## linear.sxe = read.table("data/trainingsets/2013_02_04_SXPhe.dat",header=TRUE)[,1]
...
## Apply Classifier to Linear DAta
...
save(rf.tr,file="data/linear_randomForest_KBJ.Rdat")

## Apply Classifier to Training Data
pred.train = predict(rf.tr,newdata = feat.train)
pred.train.prob = predict(rf.tr,newdata = feat.train,type='prob')

#  performance metrics
p.hat = apply(pred.train.prob,1,max)
