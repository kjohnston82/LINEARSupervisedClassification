...
source("code/color_convert_ugriz_BVR.R")
...
################################################
# add QSO class to training set
addQSO = FALSE
if(addQSO){
  qsos.sdss = read.table("data/QSO-Linear.dat")
  ind.qsos = which(ID.linear %in% qsos.sdss[,3])
  feat.train = rbind(feat.train,feat.linear[ind.qsos,])
  class.train = factor(c(paste(class.train),rep("z. QSO",length(ind.qsos))))
  ID.train = c(ID.train,ID.linear[ind.qsos])
}
...
#load("data/linear_randomForest.Rdat")
...
tab.cep = cbind(ID.linear[ind.cep],ra.dec.linear[ind.cep,1:2], p.hat[ind.cep], 1/feat.linear$freq1_harmonics_freq_0[ind.cep], feat.linear$amplitude[ind.cep])
colnames(tab.cep) = c("ID","RA","DEC","Probability","Period","Amplitude")
...
tab.cep = cbind(ID.linear[ind.mmcep],ra.dec.linear[ind.mmcep,1:2], p.hat[ind.mmcep], 1/feat.linear$freq1_harmonics_freq_0[ind.mmcep], feat.linear$amplitude[ind.mmcep])
colnames(tab.cep) = c("ID","RA","DEC","Probability","Period","Amplitude")
...
tab.mira = cbind(ID.linear[ind.mira],ra.dec.linear[ind.mira,1:2], p.hat[ind.mira], 1/feat.linear$freq1_harmonics_freq_0[ind.mira], feat.linear$amplitude[ind.mira])
colnames(tab.mira) = c("ID","RA","DEC","Probability","Period","Amplitude")
...
tab.rcb = cbind(ID.linear[ind.rcb],ra.dec.linear[ind.rcb,1:2], p.hat[ind.rcb], 1/feat.linear$freq1_harmonics_freq_0[ind.rcb], feat.linear$amplitude[ind.rcb])
colnames(tab.rcb) = c("ID","RA","DEC","Probability","Period","Amplitude")
...
tab.rrd = cbind(ID.linear[ind.rrd],ra.dec.linear[ind.rrd,1:2], p.hat[ind.rrd], 1/feat.linear$freq1_harmonics_freq_0[ind.rrd], feat.linear$amplitude[ind.rrd])
colnames(tab.rrd) = c("ID","RA","DEC","Probability","Period","Amplitude")
